...
/// Drop a CANCEL flag in `<workingDir>/.ci/runs/<runId>/CANCEL`. The
...
let invoke (workingDir: string) (runId: string) : int =
    let runsRoot = Path.Combine(workingDir, ".ci", "runs")
    let runDir = Path.Combine(Path.GetFullPath workingDir, ".ci", "runs", runId)
